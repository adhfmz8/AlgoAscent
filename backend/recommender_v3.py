...
def filter_problems_by_category_and_difficulty(
    problems: List[Problem], category: str, difficulty: str
) -> List[Problem]:
    """Filter problems by category and difficulty."""
    return [
        problem
        for problem in problems
        if problem.neetcode_category == category and problem.difficulty == difficulty
    ]


def get_solved_problems_in_category(
    attempts: List[UserAttempt], problems: List[Problem], category: str, difficulty: str
) -> int:
    ...
    return len(
        [
            ...
            in [
                p.id
                for p in problems
                if p.neetcode_category == category and p.difficulty == difficulty
            ]
            and attempt.solved
        ]
    )
...
def prioritize_problems(
    ...
    solved_easy_problems = get_solved_problems_in_category(
        attempts, problems, category, "Easy"
    )
    easy_problems_in_category = len([p for p in problems if p.difficulty == "Easy"])

    for difficulty in DIFFICULTY_PRIORITY:
        problems_with_difficulty = filter_problems_by_category_and_difficulty(
    ...
    with Session(engine) as session:
        ...
        for category in CATEGORY_PRIORITY:
            review_problems_in_category = [
                p for p in reviewable_problems if p.neetcode_category == category
            ]
        ...
        for category in CATEGORY_PRIORITY:
            unattempted_problems_in_category = [
                p for p in unattempted_problems if p.neetcode_category == category
            ]
            if unattempted_problems_in_category:
                ...
                if not has_attempt:
                    easy_problems = filter_problems_by_category_and_difficulty(
                        unattempted_problems_in_category, category, "Easy"
                    )
                    if easy_problems:
                ...
                prioritized_problems.extend(
                    prioritize_problems(
                        unattempted_problems_in_category, all_attempts, category
                    )
                )
